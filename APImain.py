    ...
    def get_watched_status():
        ...
        for user in users:
            ...
            if (user['Name'] in selectedUsers) and (user['Name'] is not None):
                ...
                api_url = '{0}Users/{1}/Items?Filters=IsPlayed&IncludeItemTypes=Movie,Episode&Recursive=True&api_key={2}'.format(
                    EMBY_URLBASE, user['Id'], EMBY_APIKEY
                )
            ...
            if eUser.replace(" ", "_") in JellyUsersIdDict.keys():
                print(
                    "Jelly already knows {0} (Id {1})".format(
                        eUser.replace(" ", "_"), JellyUsersIdDict[eUser.replace(" ", "_")]
                    )
                )
                ...
                for MigrationMedia in MigrationDataFinal[user['Name'].replace("_", " ")]:
                    if MigrationMedia['JellyId'] is not None:
                        JELLY_HEADERS_movie = {
                            ...
                            'item': json.dumps(
                                {'Name': MigrationMedia['Name'], 'Id': MigrationMedia['JellyId'], 'Played': 1},
                                separators=(',', ':'),
                            ),
                        }
                        api_url = '{0}Users/{1}/PlayedItems/{2}?api_key={3}'.format(
                            JELLY_URLBASE, user['Id'], MigrationMedia['JellyId'], JELLY_APIKEY
                        )
